...
if (isset($_REQUEST["evento"])) {
    ...
    switch ($_REQUEST["evento"]) {
        ...
        case "filtrar":
            ...
            $lessons = $controlador->buscar($campo, $modoBusqueda, $texto);
            // $lessons = $controlador->buscar($lesson);
            break;
        case "borrar":
            try {
                ...
                if (isset($_REQUEST["error"])) {
                    ...
                    $mensaje = "ERROR!!! No se ha podido borrar la lección con id: {$_REQUEST['id']}";
                }
...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h3">Buscar Lección</h1>
    </div>
    <div id="contenido">
        <div class="<?= $clase ?>" <?= $visibilidad ?> coursee="alert">
            <?= $mensaje ?>
        </div>
        <div>
        <form action="backend.php?tabla=lesson&accion=buscar&evento=filtrar" method="POST">
            <div class="form-group">
                <label for="lesson">Buscar lección</label>
                <select name="opcionesTipoBusqueda" id="opcionesTipoBusqueda">
                    ...
                    <option value="url" <?php if(isset($_POST['opcionesTipoBusqueda']) && $_POST['opcionesTipoBusqueda'] == 'url') echo 'selected="selected"'; ?>>URL</option>
                ...
                <br>
                <input type="text" required class="form-contcourse" id="busqueda" name="busqueda" value="<?= $texto ?>" placeholder="Buscar">
        ...
        <form action="backend.php?tabla=lesson&accion=buscar&evento=todos" method="POST">
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <?php
                                // $disable="";
                                $ruta="backend.php?tabla=lesson&accion=borrar&id={$id}";
                                // if (isset($lesson->esBorrable) && $lesson->esBorrable==false){
                                //     $disable="disabled"; 
                                //     $ruta="#";
                                // }
                            ?>
                            ...
                            <td><a class="btn btn-success" href="backend.php?tabla=lesson&accion=editar&id=<?=$id?>"><i class="fa fa-pencil"></i>Editar</a></td>
                        </tr>
                    <?php
                    endforeach;
