...
if (isset($_REQUEST["evento"])) {
    ...
    switch ($_REQUEST["evento"]) {
        ...
        case "filtrar":
            ...
            $students_workgroups = $controlador->buscar($campo, $modoBusqueda, $texto);
            break;
        case "borrar":
            try {
                ...
                if (isset($_REQUEST["error"])) {
                    ...
                    $mensaje = "ERROR!!! No se ha podido borrar la unión de alumno y su grupo de trabajo con student_id: {$_REQUEST['student_id']}";
                }
...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    <div id="contenido">
        ...
        <div>
        <form action="backend.php?tabla=student_workgroup&accion=buscar&evento=filtrar" method="POST">
            <div class="form-group">
                <label for="student_workgroup">Buscar Alumnos y Sus Cursos Comprados</label>
                <select name="opcionesTipoBusqueda" id="opcionesTipoBusqueda">
                    ...
                    <option value="work_group_id" <?php if(isset($_POST['opcionesTipoBusqueda']) && $_POST['opcionesTipoBusqueda'] == 'work_group_id') echo 'selected="selected"'; ?>>ID Grupo de Trabajo</option>
                </select>
        ...
        <form action="backend.php?tabla=student_workgroup&accion=buscar&evento=todos" method="POST">
            ...
            <table class="table table-light table-hover" style="text-align: center; border: 1px solid #2f3235;">
                <thead class="table-dark">
                    <tr>
                        ...
                        <th scope="col">ID Grupo de Trabajo (Nombre Grupo de Trabajo)</th>
                    </tr>
                </thead>
                <tbody>
                        ...
                        <tr>
                            ...
                            foreach ($students as $student) :
                                if ($student_workgroup->student_id == $student->id) {
                                    ?>
                                    <th scope="row"><?= $student_workgroup->student_id ?> (<?= $student->name ?>)</th>
                                    <?php
                                }
                            endforeach;

                            foreach ($workgroups as $workgroup) :
                                if (isset($student_workgroup->work_group_id) && $student_workgroup->work_group_id == $workgroup->id) {
                                    ?>
                                    <th scope="row"><?= $student_workgroup->work_group_id ?> (<?= $workgroup->name ?>)</th>
                                    <?php
                                }
                            endforeach;

                            // Define la variable $id correctamente
                            $id = isset($student_workgroup->id) ? $student_workgroup->id : null;

                            // Define la variable $disable correctamente
                            $disable = "";
                            $ruta = "backend.php?tabla=student_workgroup&accion=borrar&student_id={$student_workgroup->student_id}";
                            ?>

                            <!-- <td><a class="btn btn-danger <?= $disable ?>" href="<?= $ruta ?>"><i class="fa fa-trash"></i> Borrar</a></td> -->
                            <!-- <td><a class="btn btn-success" href="backend.php?tabla=student_workgroup&accion=editar&id=<?= $id ?>"><i class="fa fa-pencil"></i> Editar</a></td> -->
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
