...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
 ...
 <div class="d-flex justify-content-between flex-wrap flex-md-nowrap alignitems-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h3">Añadir nuevo alumno</h1>
 </div>

 <div id="contenido">
    <div class="alert alert-danger <?= $visibilidad ?>"><?= $cadena ?></div>
    <form action="backend.php?tabla=student&accion=guardar&evento=crear" method="POST">
        ...
        <div class="form-group">
          ...
          <input type="text" required class="form-control" id="nationality" name="nationality" value="<?= $_SESSION["datos"]["nationality"] ?? "" ?>" ariadescribedby="nationality" placeholder="Introduce su nacionalidad" required>
          <?= isset($errores["nationality"]) ? '<div class="alert alert-danger" role="alert">' . DibujarErrores($errores, "nationality") . '</div>' : ""; ?>
        </div>
        <br>

        <div class="form-group">
          <label for="age">Edad</label>
          <input type="number" required class="form-control" id="age" name="age" value="<?= $_SESSION["datos"]["age"] ?? "" ?>" placeholder="Introduce su edad" maxlength="2" required>
          <?= isset($errores["age"]) ? '<div class="alert alert-danger" role="alert">' . DibujarErrores($errores, "age") . '</div>' : ""; ?>
        </div>
        <br>
